...
# define variables
#coconuts
@JuMP.variable(robinson,c)
#labor
@JuMP.variable(robinson,l)
#wages
@JuMP.variable(robinson,w)
...
#@JuMP.NLconstraint(robinson,prodfunction_prime(l+1e-9)==w)

@JuMP.constraint(robinson,c>=0)
@JuMP.constraint(robinson,l>=1)
@JuMP.constraint(robinson,w>=0)
# Budget constraint in equilibrium
#@JuMP.NLconstraint(robinson,prodfunction(l+ 1e-9)==c)


# Solve model
@JuMP.objective(robinson,Min,(α1/(α2)*(Lbar-l)*w-c)^2)
